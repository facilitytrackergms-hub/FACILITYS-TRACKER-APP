    ...
    document.getElementById('createForm').onsubmit = async (e) => {
        ...
        if (file && locData.length > 0) {
            ...
            if (uploadError) {
                ...
                // Rollback: delete the location if image upload fails to keep DB clean
                await supabase.from('locations').delete().eq('id', locationId);
                alert("Upload failed. Location removed.");
                return;
            }
            ...
            if (imgError) {
                console.error("Image Table Insert Error:", imgError);
                alert("Location saved, but image failed to link: " + imgError.message);
            }
        }
